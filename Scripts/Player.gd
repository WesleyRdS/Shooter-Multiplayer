...
#variveis que vamos alterar na rede, marionetes(puppets)
...
func _ready() -> void:
	...
	username_text_instance = Global.instance_node_location(username_text, Persistent_nodes, global_position)
...
func _process(delta: float) -> void: 
	...
	if get_tree().has_network_peer():
		if is_network_master() and visible: #verifica se aquele jogado com id unico é o dono do player se não for não consegue move-lo
		...
		else: #se não for o dono do personagem faça o personagem para a posição que o personagem esta pra o dono dele
			rotation = lerp_angle(rotation, puppet_rotation, delta *8) #basicamente sicronizando a rotação de quem ta vendo o personagem do outro
			#uma extrapolação para saber a proxima posição do jogador e atualizar pra quem ta vendo:
...
func short_angle_dist(from, to): #calcula a menor distancia para girar o personagem
	var max_angle = PI * 2
...
sync func instance_bullet(id):
	var player_bullet_instance = Global.instance_node_location(player_bullet, Persistent_nodes, shoot_point.global_position)
